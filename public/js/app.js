...
function renderProducts(list, container) {
  ...
  if (!Array.isArray(list) || list.length === 0) {
    container.innerHTML = `<p class='note'>${t("products.none", "لا توجد منتجات حالياً.")}</p>`;
    return;
  }
...
function renderOffers() {
  ...
  if (!products.length) {
    offersGrid.innerHTML = `<p class='note'>${t("offers.none", "لا توجد عروض متاحة حالياً.")}</p>`;
    return;
  }
...
function renderCategories() {
  ...
  if (!categories.length) {
    categoriesGrid.innerHTML = `<p class='note'>${t("categories.none", "لا توجد أقسام متاحة حالياً.")}</p>`;
    return;
  }
...
function updateSuggestions(term, source) {
  ...
  const options = source
    ...
    .slice(0, 6)
    .map(p => `<option value="${getProductField(p, "name")}"></option>`)
    .join('');
